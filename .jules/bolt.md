...
## 2026-05-26 — Performance Optimization: Removing O(N) internal iterations during StructureAnalyzer recursive traversal

Learning:
In `StructureAnalyzer`, calling `sum(1 for p in items if p.is_file())` to compute the number of files in a directory that is already being iterated over introduces unnecessary O(N) operations.
...
## 2024-05-08 — Optimize StructureAnalyzer Directory Parsing

Learning:
`StructureAnalyzer` contained a hidden O(N) evaluation within a directory parsing loop: `sum(1 for p in items if p.is_file())` was called repeatedly for each module marker, causing O(N^2) behavior in directories with many files.
...
Learning:
Git versions reject the un-prefixed --format=COMMIT version with a 'fatal: invalid --pretty format' error. Using format: alters output semantics. tformat: must be used to ensure standard behavior.
