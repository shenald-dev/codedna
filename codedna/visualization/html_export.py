    ...
    def export(self, profile: dict, mermaid_graph: str = "") -> str:
        ...
        if profile.get("github"):
            ...
            if gh.get("is_github"):
                ...
                # Format numbers if numeric, otherwise escape string
                stars_fmt = f"{stars:,}" if isinstance(stars, (int, float)) else html_lib.escape(str(stars))
                forks_fmt = f"{forks:,}" if isinstance(forks, (int, float)) else html_lib.escape(str(forks))
                issues_fmt = f"{issues:,}" if isinstance(issues, (int, float)) else html_lib.escape(str(issues))

                github_stats = f"""
                <div class="flex items-center gap-4 mt-3 text-sm font-medium text-slate-400">
                    <span class="flex items-center gap-1"><span class="text-yellow-400">★</span> {stars_fmt}</span>  # noqa: E501
                    <span class="flex items-center gap-1"><span class="text-slate-300">⑂</span> {forks_fmt}</span>  # noqa: E501
                    <span class="flex items-center gap-1"><span class="text-green-400">⊙</span> {issues_fmt} issues</span>  # noqa: E501
                </div>
                """
        ...
        for lang, data in profile.get("languages", {}).get("languages", {}).items():
            ...
            safe_lang = html_lib.escape(str(lang))
            safe_pct = html_lib.escape(str(pct))
            safe_files = html_lib.escape(str(data['files']))
            lang_bars.append(f"""
            <div>
                <div class="flex justify-between text-sm mb-1">
                    <span class="font-medium text-slate-200">{safe_lang}</span>
                    <span class="text-slate-400">{safe_pct}% <span class="text-xs ml-1">({safe_files} files)</span></span>  # noqa: E501
                </div>
                <div class="w-full bg-slate-800 rounded-full h-2">
                    <div class="{color} h-2 rounded-full" style="width: {safe_pct}%"></div>
                </div>
            </div>
            """)
        ...
        if not risks:
            ...
            for risk in risks:
                ...
                safe_risk = html_lib.escape(str(clean_risk))
                risk_html += f'<li class="flex items-start gap-2 text-slate-300"><span class="shrink-0">{icon}</span><span>{safe_risk}</span></li>'  # noqa: E501
        ...
        devs_html = ""
        for d in devs:
            safe_name = html_lib.escape(str(d['name']))
            safe_role = html_lib.escape(str(d['role']))
            safe_commits = html_lib.escape(str(d['commits']))
            devs_html += f"""
            <div class="flex justify-between items-center bg-slate-800/50 p-2 rounded">
                <div>
                    <p class="text-sm font-medium text-slate-200">{safe_name}</p>
                    <p class="text-xs text-cyan-400">{safe_role}</p>
                </div>
                <div class="text-right">
                    <p class="text-sm font-bold text-white">{safe_commits}</p>
