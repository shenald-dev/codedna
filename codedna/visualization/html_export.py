...
<head>
    ...
    <style>
        ...
        .header-bg {{ background: linear-gradient(135deg, #1e1b4b 0%, #0f172a 100%); border-bottom: 1px solid #334155; }}
    </style>
</head>
...
<div class="header-bg py-10 px-6">
    <div class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center group">
        ...
        <div class="mt-4 md:mt-0 text-right">
            <span class="inline-block px-4 py-2 rounded-full bg-slate-800 border border-slate-700 font-mono text-cyan-400 font-semibold shadow-xl">
                {signature}
            </span>
...
<main class="max-w-7xl mx-auto px-6 py-8 grid grid-cols-1 md:grid-cols-12 gap-6">
    ...
    <div class="md:col-span-12 grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="card p-6 flex flex-col justify-center">
            <h3 class="text-slate-400 text-sm uppercase tracking-wider font-semibold mb-1">System Type</h3>
            <p class="text-2xl font-bold text-white">{system_type}</p>
        </div>
        <div class="card p-6 flex flex-col justify-center">
            <h3 class="text-slate-400 text-sm uppercase tracking-wider font-semibold mb-1">Overall Health</h3>
            <p class="text-2xl font-bold {health_color}">{health_score}</p>
        </div>
        <div class="card p-6 flex flex-col justify-center">
            <h3 class="text-slate-400 text-sm uppercase tracking-wider font-semibold mb-1">Total Files</h3>
            <p class="text-2xl font-bold text-white">{total_files}</p>
        </div>
        <div class="card p-6 flex flex-col justify-center">
            <h3 class="text-slate-400 text-sm uppercase tracking-wider font-semibold mb-1">Contributors</h3>
    ...
    <div class="md:col-span-8 flex flex-col gap-6">
        ...
        <div class="card p-6">
            ...
            <div class="bg-slate-900 rounded-lg p-4 w-full overflow-x-auto min-h-[300px] flex items-center justify-center">
                <div class="mermaid">
{mermaid_graph}
                </div>
            </div>
    ...
    <div class="md:col-span-4 flex flex-col gap-6">
        ...
        <div class="card p-6">
            ...
            <p class="text-sm text-slate-400 mb-3">Bus Factor: <strong class="text-white">{bus_factor}</strong></p>
        ...
        <div class="card p-6">
            ...
            <ul class="space-y-2 text-sm text-slate-300">
                <li class="flex justify-between"><span>Total Commits</span> <strong class="text-white">{total_commits}</strong></li>
                <li class="flex justify-between"><span>First Commit</span> <strong class="text-white">{first_commit}</strong></li>
            </ul>
...
<footer class="border-t border-slate-800 py-6 text-center text-slate-500 text-sm">
    Generated by <a href="https://github.com/shenald-dev/codedna" class="text-cyan-400 hover:underline">CodeDNA</a> — A genetic analyzer for software.
</footer>
    ...
    def export(self, profile: dict, mermaid_graph: str = "") -> str:
        ...
        if profile.get("github"):
            ...
            if gh.get("is_github"):
                ...
                <div class="flex items-center gap-4 mt-3 text-sm font-medium text-slate-400">
                    <span class="flex items-center gap-1"><span class="text-yellow-400">★</span> {gh.get('stars', 0):,}</span>
                    <span class="flex items-center gap-1"><span class="text-slate-300">⑂</span> {gh.get('forks', 0):,}</span>
                    <span class="flex items-center gap-1"><span class="text-green-400">⊙</span> {gh.get('issues', 0):,} issues</span>
                </div>
                """
        ...
        for lang, data in profile.get("languages", {}).get("languages", {}).items():
            ...
            <div>
                <div class="flex justify-between text-sm mb-1">
                    ...
                    <span class="text-slate-400">{pct}% <span class="text-xs ml-1">({data['files']} files)</span></span>
                </div>
        ...
        if not risks:
            ...
            for risk in risks:
                ...
                risk_html += f'<li class="flex items-start gap-2 text-slate-300"><span class="shrink-0">{icon}</span><span>{clean_risk}</span></li>'

        # Architecture Traits
        traits = profile.get("architecture", {}).get("traits", [])
        traits_html = "".join(f'<li class="flex items-center gap-2"><span class="text-green-400">✓</span> {t}</li>' for t in traits)
        if not traits_html:
        ...
        evo_html = "".join(f'<span class="px-2 py-1 bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded text-xs">{p}</span>' for p in evo_patterns)
        ...
        html = HTML_TEMPLATE.format(
            ...
            first_commit=profile["evolution"].get("first_commit", "")[:10] if profile["evolution"].get("first_commit") else "Unknown",
