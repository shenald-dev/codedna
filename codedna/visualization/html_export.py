    ...
    def export(self, profile: dict, mermaid_graph: str = "") -> str:
        ...
        for lang, data in profile.get("languages", {}).get("languages", {}).items():
            ...
            <div>
                <div class="flex justify-between text-sm mb-1">
                    ...
                    <span class="text-slate-400">{html_lib.escape(str(pct))}% <span class="text-xs ml-1">({html_lib.escape(str(data['files']))} files)</span></span>  # noqa: E501
                </div>
                <div class="w-full bg-slate-800 rounded-full h-2">
                    <div class="{html_lib.escape(str(color))} h-2 rounded-full" style="width: {html_lib.escape(str(pct))}%"></div>
                </div>
            </div>
            """)
        ...
        if not risks:
            ...
            for risk in risks:
                ...
                risk_html += f'<li class="flex items-start gap-2 text-slate-300"><span class="shrink-0">{icon}</span><span>{html_lib.escape(str(clean_risk))}</span></li>'  # noqa: E501
        ...
        for d in devs:
            ...
            <div class="flex justify-between items-center bg-slate-800/50 p-2 rounded">
                <div>
                    <p class="text-sm font-medium text-slate-200">{html_lib.escape(str(d['name']))}</p>
                    <p class="text-xs text-cyan-400">{html_lib.escape(str(d['role']))}</p>
                </div>
                <div class="text-right">
                    <p class="text-sm font-bold text-white">{html_lib.escape(str(d['commits']))}</p>
