...
class HTMLExporter:
    ...
    def export(self, profile: dict, mermaid_graph: str = "") -> str:
        ...
        if profile.get("github"):
            ...
            if gh.get("is_github"):
                ...
                <div class="flex items-center gap-4 mt-3 text-sm font-medium text-slate-400">
                    <span class="flex items-center gap-1"><span class="text-yellow-400">★</span> {int(gh.get('stars', 0)):,}</span>  # noqa: E501
                    <span class="flex items-center gap-1"><span class="text-slate-300">⑂</span> {int(gh.get('forks', 0)):,}</span>  # noqa: E501
                    <span class="flex items-center gap-1"><span class="text-green-400">⊙</span> {int(gh.get('issues', 0)):,} issues</span>  # noqa: E501
                </div>
                """
        ...
        for lang, data in profile.get("languages", {}).get("languages", {}).items():
            pct = html_lib.escape(str(data['percentage']))
            color = html_lib.escape(str(self._lang_tw_color(lang)))
            escaped_lang = html_lib.escape(str(lang))
            escaped_files = html_lib.escape(str(data['files']))
            lang_bars.append(f"""
            <div>
                <div class="flex justify-between text-sm mb-1">
                    <span class="font-medium text-slate-200">{escaped_lang}</span>
                    <span class="text-slate-400">{pct}% <span class="text-xs ml-1">({escaped_files} files)</span></span>  # noqa: E501
                </div>
        ...
        if not risks:
            ...
            for risk in risks:
                ...
                escaped_clean_risk = html_lib.escape(str(clean_risk))
                risk_html += f'<li class="flex items-start gap-2 text-slate-300"><span class="shrink-0">{icon}</span><span>{escaped_clean_risk}</span></li>'  # noqa: E501
        ...
        devs_html = ""
        for d in devs:
            escaped_name = html_lib.escape(str(d['name']))
            escaped_role = html_lib.escape(str(d['role']))
            escaped_commits = html_lib.escape(str(d['commits']))
            devs_html += f"""
            <div class="flex justify-between items-center bg-slate-800/50 p-2 rounded">
                <div>
                    <p class="text-sm font-medium text-slate-200">{escaped_name}</p>
                    <p class="text-xs text-cyan-400">{escaped_role}</p>
                </div>
                <div class="text-right">
                    <p class="text-sm font-bold text-white">{escaped_commits}</p>
        ...
        html = HTML_TEMPLATE.format(
            ...
            health_color=html_lib.escape(str(health_color)),
